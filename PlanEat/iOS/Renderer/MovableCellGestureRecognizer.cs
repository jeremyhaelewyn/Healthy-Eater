...
namespace PlanEat.iOS
{
	public class MovableCellGestureRecognizer : UILongPressGestureRecognizer
	{
		...
		private readonly WeakReference<MovableCell> weakCell;
		private NSIndexPath sourceOrNewAppliedIndexPath;
		private UIView opaqueView;

		private MovableCellGestureRecognizer(ListView listView, UITableView tableView, MovableCell cell, UITableViewCell nativeCell) : base(OnRecognizingAction)
		{
			...
			weakCell = new WeakReference<MovableCell>(cell);
		}

		public static MovableCellGestureRecognizer CreateGesture(ListView listView, UITableView tableView, MovableCell cell, UITableViewCell nativeCell)
		{
		...
		private void HandleRecognizerAction()
		{
			...
			switch (State)
			{
				...
				case UIGestureRecognizerState.Began:
					...
					{
						cell.BeginReorderCommand.Execute(new ReorderCommandParameters(newRowIndexPath.Row, newRowIndexPath.Section, -1, -1));
					}
				...
				case UIGestureRecognizerState.Changed:
					...
					{
						if (cell.CustomReorderCommand == null)
						{
						...
						{
							cell.CustomReorderCommand.Execute(new ReorderCommandParameters(sourceOrNewAppliedIndexPath.Row, sourceOrNewAppliedIndexPath.Section, newRowIndexPath.Row, newRowIndexPath.Section));
						}
